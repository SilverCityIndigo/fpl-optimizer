...
function XGStats({ player }) {
  ...
  return (
    <div style={{ marginTop: '8px' }}>
      <div style={{ color: '#aaa', fontSize: '10px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>xG Stats / 90</div>
      <div style={{ display: 'flex', gap: '6px' }}>
        {[['xG', xg], ['xA', xa], ['xGI', xgi]].map(([label, val]) => (
          <div key={label} style={{
            background: '#0e1117', border: `1px solid ${label === 'xGI' ? color + '55' : '#2a2f3e'}`,
            borderRadius: '5px', padding: '4px 8px', textAlign: 'center', minWidth: '44px'
          }}>
            <div style={{ color: label === 'xGI' ? color : '#fff', fontWeight: 'bold', fontSize: '13px' }}>{fmt(val)}</div>
        ...
        <div style={{ background: '#1a1f2e', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
          ...
          {viewMode === 'pitch' && picks.length > 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
              ...
              <tbody>
                {squad.map(p => (
                  <tr key={p.id} onMouseEnter={e => e.currentTarget.style.background = '#222736'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
