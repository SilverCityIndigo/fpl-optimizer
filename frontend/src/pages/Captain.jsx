        ...
        <div>
          <div style={{
            ...
            <div>
              ...
              <div style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '12px' }}>{picks[0].team_name} · {picks[0].position}</div>
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={label} style={{ background: 'var(--bg)', borderRadius: '6px', padding: '8px 14px', textAlign: 'center' }}>
                ...
                <div style={{ background: 'var(--bg)', borderRadius: '6px', padding: '8px 14px', textAlign: 'center' }}>
                  <div style={{ color: 'var(--text-secondary)', fontSize: '11px', marginBottom: '2px' }}>Fixture Difficulty</div>
                  <div style={{ color: FDR_COLORS[picks[0].fdr] || 'var(--text)', fontWeight: 'bold', fontSize: '18px' }}>
          ...
          <h3 style={{ marginBottom: '12px', fontSize: '15px', color: 'var(--text-secondary)' }}>Full Captain Rankings</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {picks.map((p, i) => (
              <div key={p.id} style={{
                background: 'var(--bg-card)', borderRadius: '8px', padding: '14px 16px',
                border: `1px solid ${i === 0 ? '#00ff87' : 'var(--border)'}`,
                display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap'
              }}>
                <div style={{ color: i === 0 ? '#00ff87' : 'var(--text-secondary)', fontWeight: 'bold', fontSize: '18px', minWidth: '28px' }}>#{i + 1}</div>
                {p.code && (
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {[['Form', p.form], ['PPG', p.points_per_game], ['Cap Pts', p.projected_captain_points], ['Fixture', p.fixture]].map(([label, value]) => (
                    <div key={label} style={{ background: 'var(--bg)', borderRadius: '6px', padding: '6px 10px', textAlign: 'center', minWidth: '55px' }}>
                      ...
                      <div style={{ color: 'var(--text)', fontWeight: 'bold', fontSize: '14px' }}>{value}</div>
                    </div>
                  ))}
                  <div style={{ background: 'var(--bg)', borderRadius: '6px', padding: '6px 10px', textAlign: 'center', minWidth: '80px' }}>
                    <div style={{ color: 'var(--text-secondary)', fontSize: '10px', marginBottom: '2px' }}>FDR</div>
                    <div style={{ color: FDR_COLORS[p.fdr] || 'var(--text)', fontWeight: 'bold', fontSize: '14px' }}>
